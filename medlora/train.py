...
def evaluate(model, loader, loss_fn, device, roi):
    ...
    for batch in loader:
        ...
        y_onehot = torch.nn.functional.one_hot(labels.long().squeeze(1), num_classes=num_classes)\
                   .permute(0,4,1,2,3).float()
        p_onehot = torch.nn.functional.one_hot(preds.long().squeeze(1), num_classes=num_classes)\
                   .permute(0,4,1,2,3).float()
        dice_metric(y_pred=p_onehot.to(device), y=y_onehot.to(device))
    return float(np.mean(losses) if losses else np.nan), float(dice_metric.aggregate().item())

def train(model, loaders, device, roi, max_epochs=100, lr=1e-4, wd=1e-4,
          early_stopping=True, patience=10, min_epochs=10, tag="run"):
    ...
    for epoch in range(1, max_epochs+1):
        model.train()
        running = []
        pbar = tqdm(train_loader, desc=f"[{tag}] epoch {epoch}/{max_epochs}",
